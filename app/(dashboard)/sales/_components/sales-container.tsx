...
const SALES_ID = '6a0e7a4ac703fabcd1cedc62';
...
export default function SalesContainer() {
  ...
  const [formData, setFormData] = useState<Omit<SalesData, '_id'>>({
    title: '',
    subTitle: '',
    dateTime: [],
    phonenumber: '',
  ...
  const salesEndpoint = useMemo(
    () => (apiBase ? `${apiBase}/sales/${SALES_ID}` : `/sales/${SALES_ID}`),
    [apiBase]
  );

  const salesQuery = useQuery({
    queryKey: ['sales-data', token],
    ...
    queryFn: async () => {
      ...
      if (!response.ok || hasExplicitFailure(payload)) {
        throw new Error(getApiMessage(payload) ?? 'Failed to load sales data.');
      }

      return payload.data as SalesData;
    },
  });
  ...
  useEffect(() => {
    if (salesQuery.data) {
      setFormData({
        title: salesQuery.data.title || '',
        subTitle: salesQuery.data.subTitle || '',
        dateTime: salesQuery.data.dateTime || [],
        phonenumber: salesQuery.data.phonenumber || '',
        description: salesQuery.data.description || '',
      });
    }
  }, [salesQuery.data]);

  const saveMutation = useMutation({
    mutationFn: async (payload: Omit<SalesData, '_id'>) => {
      const response = await fetch(salesEndpoint, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok || hasExplicitFailure(result)) {
        throw new Error(getApiMessage(result) ?? 'Failed to update sales data.');
      }
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['sales-data', token] });
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : 'Failed to save sales data.');
    },
  });
  ...
  if (status === 'unauthenticated') {
    return (
      <div className="rounded-2xl border border-red-100 bg-red-50 p-6 text-red-700">
        Please sign in first to manage Sales content.
      </div>
    );
  }

  if (status === 'loading' || salesQuery.isLoading) {
    return (
  ...
  if (salesQuery.isError) {
    return (
      <div className="rounded-2xl border border-red-100 bg-red-50 p-6 text-red-700">
        {salesQuery.error instanceof Error ? salesQuery.error.message : 'Failed to load data.'}
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-10">
      <div className="w-full">
        <section className="rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-sm">
          <div className="mb-5">
            <h2 className="text-xl font-semibold text-[#0F172A]">Sales Information</h2>
            <p className="text-sm text-[#64748B] mt-1">Manage the sales contact and schedule information displayed on the website.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={handleChange}
                  className="h-11"
                  placeholder="Looking to buy a new boiler..."
                />
              </div>
            ...
            <div className="flex gap-3 pt-4 border-t border-[#E2E8F0]">
              ...
              >
                {saveMutation.isPending ? 'Saving...' : 'Update Sales Information'}
              </Button>
            </div>
          </form>
        </section>
